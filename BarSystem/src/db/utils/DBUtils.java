...
public class DBUtils {
	...
	// return new DrinkDTO(de.getId(), de.getName(), de.getIngredients(),
	// de.getPrice());
	// }
	//
	...
	// {
	// RoleDTO r = new RoleDTO(ce.getUserId().getRole().getId(),
	// ce.getUserId().getRole().getRoleName());
	// UserDTO u = new UserDTO(ce.getUserId().getId(), ce.getUserId().getName(),
	// ce.getUserId().getPassword(), r);
	...
	// RoleDTO rd = new
	// RoleDTO(oe.getConsumerId().getUserId().getRole().getId(),
	// oe.getConsumerId().getUserId().getRole().getRoleName());
	// UserDTO ud = new UserDTO(oe.getConsumerId().getUserId().getId(),
	// oe.getConsumerId().getUserId().getName(),
	// oe.getConsumerId().getUserId().getPassword(), rd);
	// ConsumerDTO cd = new ConsumerDTO(oe.getConsumerId().getId(),
	// oe.getConsumerId().getDate(), oe.getConsumerId().getPlace(), ud);
	// Map<DrinkDTO, Integer> drinksMap = new HashMap<DrinkDTO,
	// Integer>(oe.getDrinks().size());
	// for(Entry<DrinkEntity, Integer> e : oe.getDrinks().entrySet()) {
	// drinksMap.put(DBUtils.DrinkEntityToDrinkDto(e.getKey()), e.getValue());
	// }
	// return new OrderDTO(oe.getId(), cd, drinksMap, oe.getStatus(),
	// oe.getBill());
	// }
