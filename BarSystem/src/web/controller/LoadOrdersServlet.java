	...
	private void createTable(User user, List<Order> ordersList, PrintWriter out) {
		...
		for(Order o : ordersList) {
			...
			if ((user.getRole().getRole().equals(RolesType.WAITER) && !o.getStatus().equals(OrderStatus.DONE)))
					/*|| (user.getRole().getRole().equals(RolesType.BARMAN) && (o.getStatus().equals(OrderStatus.DONE) || o.getStatus().equals(OrderStatus.ACCEPTED))))*/ { // so it is "Pending"/"Overdue"/"Accepted"
			...
			out.print("<td> ...pitieta... </td>");
			out.print("<td> ...broi...</td>");
