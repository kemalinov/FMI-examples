...
@NamedNativeQueries({
		...
		@NamedNativeQuery(name = "OrderEntity.getOrdersForActiveConsumersByUserId", query = "SELECT od.order_id, od.consumer_consumer_id, c.place, d.name, odd.drinks, od.status, od.bill "
				+ "FROM app.orders od " + "JOIN app.consumers c " + "ON c.closed = false AND od.consumer_consumer_id = c.consumer_id  AND c.user_user_id = ? " + "JOIN app.ordered_drinks odd "
				+ "ON odd.orderentity_order_id = od.order_id " + "JOIN app.drinks d " + "ON d.drink_id = odd.drinks_key", resultClass = OrderEntity.class),
